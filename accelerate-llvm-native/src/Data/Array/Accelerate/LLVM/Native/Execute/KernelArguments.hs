...
prepareKernel env (NativeKernelMetadata envSize) fun args = do
  ...
  withForeignPtr foreignPtr $ \argPtr -> do
    ...
    let
      ...
      go cursor (KernelFunBody (NativeKernel funLifetime _ _ _ _ _)) ArgsNil
...
touchKernel env = go
  where
    ...
    go (KernelFunBody (NativeKernel funLifetime _ _ _ _ _)) ArgsNil = touchLifetime funLifetime
