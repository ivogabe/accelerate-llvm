...
convert inAwhile (Effect effect@(Exec _ kernel kargs) next)
  ...
  Exists2 $ Phase1{
    ...
    phase2 = \imports fullState structVars localVars importsIdx stateIdx nextBlock -> do
      ...
      nextShardPtr <- instr' $ GetElementPtr $ gepStruct primType args (TupleIdxLeft $ TupleIdxLeft $ TupleIdxRight TupleIdxSelf)
      _ <- instr' $ Store NonVolatile nextShardPtr (integral TypeWord64 0) Nothing -- next shard to work on
      -- Arguments
